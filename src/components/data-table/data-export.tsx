...
export function DataTableExport<TData>({
  ...
  const handleExport = async (type: 'csv' | 'excel') => {
    ...
    try {
      // If transformData and configHeaders are provided, use them for a clean export
      if (transformData && configHeaders && columnMapping) {
      ...
      // Fallback: Get visible columns from the table
  ...
  const exportAllPages = async (type: 'csv' | 'excel') => {
    ...
    try {
      ...
      // If transformData and configHeaders are provided, use them for a clean export
      if (transformData && configHeaders && columnMapping) {
        const exportData = transformData(allData);

        let success: boolean;
        if (type === 'csv') {
          success = exportToCSV(exportData, filename, configHeaders, columnMapping);
        } else {
        ...
        if (success) {
          toast.success('Export successful', {
            description: `Exported ${allData.length} ${entityName} to ${type.toUpperCase()}.`,
            id: 'export-data-toast',
          });
        }
        return;
      ...
      // Fallback: Get visible columns and apply export
      const visibleColumns = table
        .getAllColumns()
        .filter(column => column.getIsVisible())
        .filter(column => column.id !== 'actions' && column.id !== 'select');
